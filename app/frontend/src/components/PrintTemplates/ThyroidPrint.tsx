    ...
    <div className="space-y-4">
      ...
      {data.right_length && (
        <div>
          ...
          <p>
            Размеры: {data.right_length} x {data.right_width} x{" "}
            {data.right_thickness} мм.
            {data.right_contours && ` Контуры: ${data.right_contours}.`}
            {data.right_echogenicity &&
              ` Эхогенность: ${data.right_echogenicity}.`}
            {data.right_structure && ` Структура: ${data.right_structure}.`}
            {data.right_nodules === "есть" &&
              ` Узлы: ${data.right_nodule_size}, ${data.right_nodule_echo}.`}
          </p>
        </div>
      )}
      ...
      {data.left_length && (
        <div>
          ...
          <p>
            Размеры: {data.left_length} x {data.left_width} x{" "}
            {data.left_thickness} мм.
            {data.left_contours && ` Контуры: ${data.left_contours}.`}
            {data.left_echogenicity &&
              ` Эхогенность: ${data.left_echogenicity}.`}
            {data.left_structure && ` Структура: ${data.left_structure}.`}
            {data.left_nodules === "есть" &&
              ` Узлы: ${data.left_nodule_size}, ${data.left_nodule_echo}.`}
          </p>
        </div>
      )}
